...
static int get_client_ch1_range(ws_cli_conn_t *client)
{
	int state;
	...
	pthread_mutex_lock(&client->mtx_state);
	state = client->ch1_range;
	pthread_mutex_unlock(&client->mtx_state);
	return (state);
}

static int get_client_ch2_range(ws_cli_conn_t *client)
{
	int state;
	...
	pthread_mutex_lock(&client->mtx_state);
	state = client->ch2_range;
	pthread_mutex_unlock(&client->mtx_state);
	return (state);
}

static void set_client_ch1_range(ws_cli_conn_t *client, int range)
{
	if (!CLIENT_VALID(client))
		return (-1);

	pthread_mutex_lock(&client->mtx_state);
	client->ch1_range = range;
	pthread_mutex_unlock(&client->mtx_state);
}

static void set_client_ch2_range(ws_cli_conn_t *client, int range)
{
	if (!CLIENT_VALID(client))
		return (-1);

	pthread_mutex_lock(&client->mtx_state);
	client->ch2_range = range;
	pthread_mutex_unlock(&client->mtx_state);
}
...
int ws_get_ch1_range(ws_cli_conn_t *client)
{
	return (get_client_ch1_range(client));
}

int ws_get_ch2_range(ws_cli_conn_t *client)
{
	return (get_client_ch2_range(client));
}

void ws_set_ch1_range(ws_cli_conn_t *client, int range)
{
	set_client_ch1_range(client, range);
}

void ws_set_ch2_range(ws_cli_conn_t *client, int range)
{
	set_client_ch2_range(client, range);
}
	...
	while (1)
	{
		...
		for (i = 0; i < MAX_CLIENTS; i++)
		{
			if (client_socks[i].client_sock == -1)
			{
				...
				client_socks[i].ch1_range = -1;
				client_socks[i].ch2_range = -1;
